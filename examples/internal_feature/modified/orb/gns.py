...
class MoleculeGNS(nn.Module):
    ...
    def forward(
        self, batch: base.AtomGraphs, return_intermediate: bool = False
    ) -> base.AtomGraphs:
        ...
        if return_intermediate:
            intermediate = {
                "node_emb": [batch.node_features[_KEY]],
                "edge_emb": [batch.edge_features[_KEY]],
            }

        for idx, gnn in enumerate(self.gnn_stacks):
            batch = gnn(batch)
            if return_intermediate:
                intermediate[f"node_attr_{idx}"] = batch.node_features[_KEY]
                intermediate[f"edge_attr_{idx}"] = batch.edge_features[_KEY]

        batch = self._decoder(batch)
        if return_intermediate:
            intermediate["pred"] = batch.node_features["pred"]
            return batch, intermediate
        else:
            return batch
