...
class MatglDataset(MatterTuneDatasetBase):
    ...
    def __getitem__(self, idx: int) -> DataTypedDict:
        ...
        data = DataTypedDict(
            ...
            energy=labels.get(
                "energy", torch.tensor([0.0], dtype=matgl.float_th)
            ).reshape(1),
            force=labels.get("force", torch.zeros_like(pos)),
            stress=labels.get("stress", torch.zeros((1, 3, 3), dtype=matgl.float_th)),
        )
        return data
...
class MatglDataModule(MatterTuneDataModuleBase):
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        train_labels = {
            k: [v[i] for i in indices[:train_end]] for k, v in self.labels.items()
        }
        val_labels = {
            k: [v[i] for i in indices[train_end:val_end]]
            for k, v in self.labels.items()
        }
        test_labels = {
            k: [v[i] for i in indices[val_end:]] for k, v in self.labels.items()
        }
    ...
    def collate_fn(self, data_list: list[DataTypedDict]) -> BatchTypedDict:
        ...
        energy = torch.cat([data["energy"] for data in data_list])
        force = torch.cat([data["force"] for data in data_list])
        stress = torch.cat([data["stress"] for data in data_list])
        batch_idx = torch.cat(
            [
