...
# Schema entries
class ManualSplitDataModuleConfigTypedDict(typ.TypedDict, total=False):
...
@typ.overload
def CreateManualSplitDataModuleConfig(
    dict: ManualSplitDataModuleConfigTypedDict, /
) -> ManualSplitDataModuleConfig: ...


@typ.overload
def CreateManualSplitDataModuleConfig(
    **dict: typ.Unpack[ManualSplitDataModuleConfigTypedDict],
) -> ManualSplitDataModuleConfig: ...


def CreateManualSplitDataModuleConfig(*args, **kwargs):
    from mattertune.data.datamodule import ManualSplitDataModuleConfig

    dict = args[0] if args else kwargs
    return ManualSplitDataModuleConfig.model_validate(dict)
