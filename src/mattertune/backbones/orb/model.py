...
class EdgeEnergyHead(EnergyHead):
    ...
    def forward(
        ...
        pred = self.mlp(torch.cat([sender_features, receiver_features], dim = 1))#torch.cat([edge_features, sender_features, receiver_features, direct_features], dim = 1))
        return pred.squeeze(-1)
        ...
        match prop:
            ...
            case props.EdgeInvariantVectorPropertyConfig():
                ...
                if not self.hparams.reset_output_heads:
                    ...
                    head = EdgeEnergyHead(
                        latent_dim=256*2,#*3 + 3,
                        num_mlp_layers=num_layers,
                        mlp_hidden_dim=hidden_dim,
                    )
                    head.mlp = build_mlp(
                        input_size=256*2,#*3 + 3,
                        hidden_layer_sizes=[hidden_dim] * num_layers,
                        output_size=prop.size,
                        activation='silu',
                        dropout=prop.additional_head_settings['dropout_rate'] / 100.0,
