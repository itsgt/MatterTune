...
class GlobalBinaryClassificationOutputHeadConfig(OutputHeadBaseConfig):
    ...
    def construct_output_head(
        ...
        if self.pos_weight is not None:
            assert (
                self.pos_weight > 0
            ), f"pos_weight of GlobalBinaryClassificationOutputHead must be positive, found {self.pos_weight}"
        if self.hidden_dim is None:
            raise ValueError(
                "hidden_dim must be provided for GlobalBinaryClassificationOutputHead"
            )
...
class GlobalMultiClassificationOutputHeadConfig(OutputHeadBaseConfig, Generic[TBatch]):
    ...
    def construct_output_head(
        ...
        if self.class_weights is not None:
            ...
            ), f"Number of class weights must match the number of classes, found {len(self.class_weights)} != {self.num_classes}"
            for weight in self.class_weights:
                assert weight > 0, f"Class weights must be positive, found {weight}"
        if self.dropout is not None:
            assert (
                0.0 <= self.dropout < 1.0
            ), f"Dropout probability must be between 0 and 1, found {self.dropout}"
        if self.hidden_dim is None:
            raise ValueError(
                "hidden_dim must be provided for GlobalMultiClassificationOutputHead"
            )
    ...
    def forward(
        ...
        *,
        ...
        assert scaler.shape == (
            num_graphs,
            1,
        ), f"scaler.shape={scaler.shape} != [num_graphs, 1]"
    ...
    def forward(
        ...
        *,
        ...
        assert logits.shape == (
            num_graphs,
            1,
        ), f"logits.shape={logits.shape} != [num_graphs, 1]"
...
class GlobalMultiClassificationOutputHead(nn.Module, Generic[TBatch]):
    ...
    def __init__(
        ...
        self.out_mlp = MLP(
            ([self.hidden_dim] * self.head_config.num_layers)
    ...
    def forward(
        ...
        *,
        ...
        assert (
            logits.shape == (num_graphs, self.head_config.num_classes)
        ), f"logits.shape={logits.shape} != [num_graphs, {self.head_config.num_classes}]"
        output_head_results[self.head_config.target_name] = logits
        return logits
