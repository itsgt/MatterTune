            ...
            for i, struct_i_flt in enumerate(label):
                ...
                for j in range(len(unique_abs)):
                    ...
                    for p_idx in range(len(unique_paths)):
                        ...
                        assert degen_all[abs_mask][p_mask][0].int() == p_mask.sum().int(), f'Expected degen of {degen_all[abs_mask][p_mask][0].int()} but got {p_mask.sum().int()}. Reff = {Reff_all[abs_mask][p_mask][0]} edge_vecs = {batch.edge_features["vectors"][edge_slice][valid][abs_mask][p_mask]}'
